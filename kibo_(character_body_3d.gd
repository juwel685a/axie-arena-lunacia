...
@export var health_bar_visible_time: float = 4.0
...
func handle_wander(delta: float) -> void:
	...
	if distance_from_home > wander_radius:
		var direction_home: Vector3 = (
			home_position - global_position
		)

		direction_home.y = 0.0

		if direction_home.length() > 0.1:
			move_wander_direction(
				direction_home.normalized(),
				delta
			)
...
func get_avoidance_direction(
	...
	if right_clear and left_clear:
		obstacle_avoid_direction = right_direction
	...
	if left_clear:
		obstacle_avoid_direction = left_direction
		obstacle_avoid_timer = 0.7
	...
	if not is_direction_blocked(
		...
		obstacle_avoid_direction = diagonal_right
	...
	if not is_direction_blocked(
		...
		obstacle_avoid_direction = diagonal_left
		obstacle_avoid_timer = 0.8
...
func take_damage(amount: float) -> void:
	...
	health_bar_visible_timer = health_bar_visible_time
